...
from app.serializers import UserBaseSerializer, RoleBaseSerializer, ProjectProfileSerializer, ModelProfileSerializer
...
class ProjectProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Project_Profile.objects.all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = self.serializer_class(data=data)
        if self.serializer_class().is_project_exist(data):
            msg = _('Project already exists!')
            raise serializers.ValidationError({'detail': msg})
    ...
    def update(self, request, *args, **kwargs):
        ...
        if self.serializer_class().is_project_exist(request.data):
            msg = _('Project already exists!')
            raise serializers.ValidationError({'detail': msg})
...
class ModelProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Model_Profile.objects.all()
        name = self.request.query_params.get('name', None)
        if name is not None:
            queryset = queryset.filter(name__icontains=name)
        return queryset
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = self.serializer_class(data=data)
        if self.serializer_class().is_model_exist(data):
            msg = _('Model already exists!')
            raise serializers.ValidationError({'detail': msg})
    ...
    def update(self, request, *args, **kwargs):
        ...
        if self.serializer_class().is_model_exist(request.data):
            msg = _('Model already exists!')
            raise serializers.ValidationError({'detail': msg})
...
class MLViewSet(views.APIView):
    def post(self, request, *args, **kwargs):
        ...
        return Response({'result': prediction[0][0]})